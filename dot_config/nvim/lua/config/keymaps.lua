...
vim.api.nvim_create_autocmd("LspAttach", {
	...
	callback = function(event)
		-- Go to definition
		keymap.set("n", "gd", "<cmd>lua vim.lsp.buf.definition()<cr>", { buffer = event.buf })
		...
		--keymap.set("n", "K", "<cmd>lua vim.lsp.buf.hover()<cr>", { buffer = event.buf })

		-- Format code
		--keymap.set("n", "F", "<cmd>lua vim.lsp.buf.format()<cr>", { buffer = event.buf })

		-- Rename symbol
		--keymap.set("n", "3r", "<cmd>lua vim.lsp.buf.rename()<cr>", { buffer = event.buf })
	end,
})
