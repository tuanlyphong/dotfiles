...
Item {
    ...
    function resetFocus(text, key) {
        var pendingText = text || "";
        var pendingKey = key || 0;
        // Backspace/Enter/etc. have no meaningful "text" to replay and
        // would just clutter the field, so only forward printable text.
        var shouldForward = pendingText !== "" && pendingKey !== Qt.Key_Backspace && pendingKey !== Qt.Key_Return && pendingKey !== Qt.Key_Enter;

        if (!loginScreen.foundUsers) {
            userInput.input.forceActiveFocus();
            if (shouldForward) {
                userInput.text += pendingText;
            }
        } else {
            if (loginScreen.userNeedsPassword) {
                password.input.forceActiveFocus();
                if (shouldForward) {
                    password.text += pendingText;
                }
            } else {
