...
Item {
    id: lockScreen
    // Carries the text/key of whatever keystroke triggered the
    // transition to the login screen, so that keystroke isn't lost.
    signal loginRequested(string text, int key)
    ...
    MouseArea {
        ...
        onClicked: lockScreen.loginRequested("", 0)
    }
    ...
    Keys.onPressed: function (event) {
        ...
        if (event.key === Qt.Key_Escape) {
            ...
            // Forward the triggering keystroke along so the login
            // screen can replay it into the password field instead
            // of losing it during the focus transition.
            lockScreen.loginRequested(event.text, event.key);
        }
        event.accepted = true;
    }
}
